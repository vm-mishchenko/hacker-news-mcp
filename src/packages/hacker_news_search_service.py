...
Function flow:
search (coordinator)
├── _run_vector_search (coordinator)
│   └── embedding_service.generate_embedding (external)
├── _extract_doc_ids (executor)
└── _run_boosted_text_search (executor)
"""
...
class HackerNewsSearchService:
    ...
    def __init__(
        ...
        Args:
            ...
            embedding_service: OpenAI embedding service for vector search
        """
    ...
    def _run_text_search(self, query: str, boost_doc_ids: List[int], limit: int) -> List[
        HackerNewsStory]:
        """Execute text search with boosting for specific document IDs and story score.
        ...
        The "should" clauses boost documents from vector search with decreasing weights.
        """
        logger.info(
